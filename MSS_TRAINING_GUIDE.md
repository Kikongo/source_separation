...
example:
MUSDB_PATH = "/source_separation/data/"

### Training a 2-Source Separation Model

```bash
python mss_train.py --config-name=mss_2sources
```

### Training a 4-Source Separation Model

```bash
python mss_train.py --config-name=mss_4sources
```
...
- `mss_2sources.yaml` - Configuration for 2-source separation (vocals + accompaniment)
- `mss_4sources.yaml` - Configuration for 4-source separation (vocals, drums, bass, other)
