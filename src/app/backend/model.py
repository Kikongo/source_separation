...
class SpectrogramChannelsUNet(nn.Module):
    def __init__(self, n_sources=4):
        ...
        #Batchnorm decoder
        self.batch_norm_dec1_1 = nn.BatchNorm2d(256)
        self.batch_norm_dec1_2 = nn.BatchNorm2d(256)
        self.batch_norm_dec1_3 = nn.BatchNorm2d(256)

        self.batch_norm_dec2_1 = nn.BatchNorm2d(128)
        self.batch_norm_dec2_2 = nn.BatchNorm2d(128)
        self.batch_norm_dec2_3 = nn.BatchNorm2d(128)

        self.batch_norm_dec3_1 = nn.BatchNorm2d(64)
        self.batch_norm_dec3_2 = nn.BatchNorm2d(64)
        self.batch_norm_dec3_3 = nn.BatchNorm2d(64)

        self.batch_norm_dec4_1 = nn.BatchNorm2d(32)
        self.batch_norm_dec4_2 = nn.BatchNorm2d(32)
        self.batch_norm_dec4_3 = nn.BatchNorm2d(32)
        ...
        self.batch_norm_center1 = nn.BatchNorm2d(512)
        self.batch_norm_center2 = nn.BatchNorm2d(512)
    ...
    def forward(self, mixture_spectrogram):
        ...
        e1_1 = self.relu(self.batch_norm1_1(self.enc1_1(mixture_spectrogram)))
        e1_2 = self.relu(self.batch_norm1_2(self.enc1_2(e1_1)))
        ...
        e2_1 = self.relu(self.batch_norm2_1(self.enc2_1(max_pool_e1)))
        e2_2 = self.relu(self.batch_norm2_2(self.enc2_2(e2_1)))
        max_pool_e2 = self.max_pool(e2_2)

        #print(f"Enc2_1 {e2_1.shape}")
        #print(f"MaxPoolE2 {max_pool_e2.shape}")

        e3_1 = self.relu(self.batch_norm3_1(self.enc3_1(max_pool_e2)))
        e3_2 = self.relu(self.batch_norm3_2(self.enc3_2(e3_1)))
        max_pool_e3 = self.max_pool(e3_2)

        #print(f"Enc3_1 {e3_1.shape}")
        #print(f"MaxPoolE3 {max_pool_e3.shape}")

        e4_1 = self.relu(self.batch_norm4_1(self.enc4_1(max_pool_e3)))
        e4_2 = self.relu(self.batch_norm4_2(self.enc4_2(e4_1)))
        ...
        #Bottleneck
        b = self.relu(self.batch_norm_center1(self.bottleneck(max_pool_e4)))
        #print(f"bottlenec {b.shape}")
        b = self.relu(self.batch_norm_center2(self.bottleneck2(b)))
        #print(f"bottlenec {b.shape}")

        #Обратный проход
        d1_1 = self.dropout(self.relu(self.batch_norm_dec1_1(self.dec1_1(b))))
        #print(f"dec1_1 {d1_1.shape}")
        concat_d1 = torch.cat((d1_1, e4_2), dim=1)
        #print(f"Concat d1: {concat_d1.shape}")
        d1_2 = self.relu(self.batch_norm_dec1_2(self.dec1_2(concat_d1)))
        #print(f"dec1_2 {d1_2.shape}")
        d1_3 = self.relu(self.batch_norm_dec1_3(self.dec1_3(d1_2)))
        #print(f"dec1_3 {d1_3.shape}")

        d2_1 = self.dropout(self.relu(self.batch_norm_dec2_1(self.dec2_1(d1_3)))) 
        concat_d2 = torch.cat((d2_1, e3_2), dim=1)
        d2_2 = self.relu(self.batch_norm_dec2_2(self.dec2_2(concat_d2)))  # Skip connection
        d2_3 = self.relu(self.batch_norm_dec2_3(self.dec2_3(d2_2)))
        #print(f"dec2 {d2_1.shape}")

        d3_1 = self.dropout(self.relu(self.batch_norm_dec3_1(self.dec3_1(d2_3))))  
        concat_d3 = torch.cat((d3_1, e2_2), dim=1)
        d3_2 = self.relu(self.batch_norm_dec3_2(self.dec3_2(concat_d3)))  # Skip connection
        d3_3 = self.relu(self.batch_norm_dec3_3(self.dec3_3(d3_2)))  
        #print(f"dec3 {d3_1.shape}")

        d4_1 = self.dropout(self.relu(self.batch_norm_dec4_1(self.dec4_1(d3_3))))  
        concat_d4 = torch.cat((d4_1, e1_2), dim=1)
        d4_2 = self.relu(self.batch_norm_dec4_2(self.dec4_2(concat_d4)))  # Skip connection
        d4_3 = self.relu(self.batch_norm_dec4_3(self.dec4_3(d4_2)))
        #print(f"dec4 {d4_1.shape}")

        # Выход: маски или спектрограммы для всех источников
        output = self.relu(self.output(d4_3))
